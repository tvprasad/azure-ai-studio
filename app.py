...
# Approximate Azure retail pricing (USD, 2026-Q1). See azure.com/pricing for production billing.
_GOVERNANCE_PRICING: dict[str, float] = {
    "Language:SentimentAnalysis":   0.001,     # $1 / 1k text records
    "Language:EntityRecognition":   0.001,
    "Language:KeyPhraseExtraction": 0.001,
    "Language:LanguageDetection":   0.001,
    "Vision:ImageAnalysis":         0.001,     # $1 / 1k transactions
    "Speech:FastTranscribe":        0.017,     # ~$1/hr ≈ $0.017 / min (1-min unit)
    "Speech:Synthesize":            0.000004,  # $4 / 1M chars (per-character)
    "DocIntel:prebuilt-read":       0.0015,    # $1.50 / 1k pages (1-page unit)
    "DocIntel:poll":                0.0,
}
...
class GovernanceLayer:
    """
    AI Governance: cost estimation and session-level usage tracking.
    Pricing is approximate retail; not a substitute for Azure billing data.
    """

    @staticmethod
    def enrich(meta: AzureCallMeta, units: int = 1) -> None:
        """Attach an estimated cost to the meta object (mutates in place)."""
        meta.estimated_cost_usd = _GOVERNANCE_PRICING.get(meta.kind, 0.0) * units

    @staticmethod
    def record(meta: AzureCallMeta) -> None:
        """Accumulate per-service call counts and cost into session state."""
        if "governance" not in st.session_state:
            st.session_state.governance = {}
        service = meta.kind.split(":")[0]
        g = st.session_state.governance
        if service not in g:
            g[service] = {"calls": 0, "cost_usd": 0.0}
        g[service]["calls"] += 1
        g[service]["cost_usd"] += meta.estimated_cost_usd
...
class AzureSpeechClient:
    ...
    def fast_transcribe(self, audio_bytes: bytes, locale: str = "en-US") -> tuple[dict, AzureCallMeta]:
    ...
    def synthesize(self, ssml: str, output_format: str = "audio-16khz-32kbitrate-mono-mp3") -> tuple[bytes, AzureCallMeta]:
        ...
        meta = AzureCallMeta(
            ...
            status_code=resp.status_code,
            elapsed_ms=elapsed_ms,
            endpoint=f"https://{self.region}.tts.speech.microsoft.com",
        )
...
with st.sidebar.expander("Diagnostics"):
    if st.session_state.telemetry:
        ...
        st.write(f"Est. Cost: ${last.estimated_cost_usd:.5f}")
        st.write(f"Total Calls: {len(st.session_state.telemetry)}")
        st.caption(f"Request ID: {last.request_id}")
    else:
        st.caption("No API calls yet.")

with st.sidebar.expander("Governance"):
    if st.session_state.governance:
        total_calls = sum(v["calls"] for v in st.session_state.governance.values())
        total_cost = sum(v["cost_usd"] for v in st.session_state.governance.values())
        col_a, col_b = st.columns(2)
        col_a.metric("Total Calls", total_calls)
        col_b.metric("Session Cost", f"${total_cost:.4f}")
        st.markdown("**Usage by Service**")
        for svc, data in st.session_state.governance.items():
            st.write(f"• **{svc}**: {data['calls']} calls — ${data['cost_usd']:.4f}")
        st.caption("Estimates based on Azure retail pricing (2026-Q1). Not actual billing data.")
    else:
        st.caption("No API calls yet.")
        ...
        def execute(kind, renderer):
            try:
                with st.spinner("Analyzing..."):
                    ...
                    GovernanceLayer.enrich(meta)
                    GovernanceLayer.record(meta)
                    st.session_state.telemetry.append(meta)
                    renderer(result)
                    st.caption(f"{meta.status_code} • {meta.elapsed_ms} ms • est. ${meta.estimated_cost_usd:.5f} • {meta.request_id}")
            ...
            if audio_file and st.button("Transcribe"):
                try:
                    with st.spinner("Transcribing..."):
                        result, meta = speech.fast_transcribe(audio_file.getvalue(), locale="en-US")
                    GovernanceLayer.enrich(meta, units=1)  # billed per minute; 1-min estimate
                    GovernanceLayer.record(meta)
                    st.session_state.telemetry.append(meta)
                    st.json(result)
                    st.caption(f"{meta.status_code} • {meta.elapsed_ms} ms • est. ${meta.estimated_cost_usd:.5f} • {meta.request_id}")
        ...
        with tab_tts:
            ...
            if st.button("Generate Audio"):
                try:
                    ...
                    audio_bytes, meta = speech.synthesize(ssml)
                    GovernanceLayer.enrich(meta, units=len(text))  # billed per character
                    GovernanceLayer.record(meta)
                    st.session_state.telemetry.append(meta)
                    st.audio(audio_bytes, format="audio/mp3")
                    st.caption(f"Voice: {voice_name} • {meta.elapsed_ms} ms • est. ${meta.estimated_cost_usd:.5f}")
                except AzureServiceError as e:
                    st.error(str(e))
        ...
        if up:
            ...
            if st.button("Extract Text (prebuilt-read)"):
                try:
                    ...
                    GovernanceLayer.enrich(meta, units=1)  # billed per page; 1-page estimate
                    GovernanceLayer.record(meta)
                    st.session_state.telemetry.append(meta)
                    st.caption(f"{meta.status_code} • {meta.elapsed_ms} ms • est. ${meta.estimated_cost_usd:.5f} • {meta.request_id}")
